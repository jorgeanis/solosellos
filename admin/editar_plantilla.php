...
$id = $_GET['id'] ?? null;
if (!$id) {
    die("<p style='color:red'>ID de plantilla no especificado.</p>");
}
...
// Carga de datos para el formulario (GET)
$stmt = $pdo->prepare("SELECT * FROM templates WHERE id = ? AND user_id = ?");
$stmt->execute([$id, $_SESSION['user']['id']]);
$plantilla = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$plantilla) {
    die("<p style='color:red'>Plantilla no encontrada.</p>");
}
...
<h2>Editando plantilla: <?= htmlspecialchars($plantilla["nombre"]) ?></h2>
